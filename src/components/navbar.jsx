...
const Navbar = () => {
  ...
  return (
    <div className='h-full flex items-center px-4 sm:px-8 md:px-12 lg:px-20 lx:px-48 text-xl'>

      <div className="w-full flex justify-between">

        {/* LOGO */}
        <div className="flex">
          <Link href="/" className=" bg-black rounded-md p-2 flex items-center justify-center">
            <span className="text-white">SANJAM KHERA</span>
          </Link>
        </div>

        {/* LINKS */}
        <div className='hidden md:flex w-1/4 justify-between mr-6'>
          {links.map((link) => (<NavLink link={link} key={link.title}>{link.title}</NavLink>))}
        </div>
      ...
      <div className="md:hidden">
        ...
        {open && (
          <motion.div variants={listVariants} initial="closed" animate="opened"
            ...
            {links.map(link => (<motion.div variants={listItemVariants} className='' key={link.title}><Link href={link.url}>{link.title}</Link></motion.div>))}</motion.div>)}
